    ...
    fn try_from(s: &str) -> Result<Self, Self::Error> {
        match &s[0..2] {
            ...
            "u;" => {
                ...
                Ok(Update(data_hash_arr[0].to_string(), data_hash_arr[1].to_string(), timestamp))
            }
            "d;" => {
                let arr: Vec<_> = s[2..].split(';').collect();
                if arr.len() != 2 {
...
impl Display for OpEnum {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Insert(s, t) => {
                f.write_str(&format!("i;{};{}", s, t))
            }
            Update(old, new, t) => {
                f.write_str(&format!("u;{}|{};{}", old, new, t))
            }
            Delete(s, t) => {
                f.write_str(&format!("d;{};{}", s, t))
            }
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn test_ser() {
        let i = Insert("1".to_string(), Timestamp::from_str("1970-01-01T00:00:00.000000000Z/1").unwrap()).to_string();
        assert_eq!(i, "i;1;1970-01-01T00:00:00.000000000Z/1".to_string());

        let i = Update("1".to_string(), "2".to_string(), Timestamp::from_str("1970-01-01T00:00:00.000000000Z/1").unwrap()).to_string();
        assert_eq!(i, "u;1|2;1970-01-01T00:00:00.000000000Z/1".to_string());

        let d = Delete("1".to_string(), Timestamp::from_str("1970-01-01T00:00:00.000000000Z/1").unwrap()).to_string();
    ...
    fn test_des() {
        ...
        assert_eq!(i, Insert("1".to_string(), Timestamp::from_str("1970-01-01T00:00:00.000000000Z/1").unwrap()));

        let u: OpEnum = OpEnum::try_from("u;1|2;1970-01-01T00:00:00.000000000Z/1").unwrap();
        assert_eq!(u, Update("1".to_string(), "2".to_string(), Timestamp::from_str("1970-01-01T00:00:00.000000000Z/1").unwrap()));

        let d: OpEnum = OpEnum::try_from("d;1;1970-01-01T00:00:00.000000000Z/1").unwrap();
        assert_eq!(d, Delete("1".to_string(), Timestamp::from_str("1970-01-01T00:00:00.000000000Z/1").unwrap()));
    }
}
